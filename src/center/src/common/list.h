 ...
 * 3. add macro offsetof2() and container_of
 *
...
static inline int list_size(const struct list_head *entry)
{
    int i = 0;
...
#define list_queue_push(pnew, head)  list_add_tail(pnew, head)
#define list_stack_push(pnew, head)  list_add(pnew, head)
...
static inline struct list_head *list_get (int index, struct list_head *entry)
{
    int i = 0;
    struct list_head *it;

    list_for_each (it, entry) {
        if (i == index) {
            return it;
        }
        ++i;
...
/* insert the pnew node to after it*/
static inline void list_insert_after (struct list_head *pnew, struct list_head *it)
{
    //TODO:
}

/* insert the pnew node befault it */
static inline void list_insert_before (struct list_head *pnew, struct list_head *it)
{
    it->prev->next = pnew;
    pnew->prev = it->prev;
    pnew->next = it;
